...
use skju_core::filter::simple_filter::SimpleFilter;
...
fn main() {
    ...
    let sensors: Vec<Arc<Mutex<Sensor<SimpleFilter>>>> = sensors
        .into_iter()
...
fn create_sensor_from_config(sensor_config: SensorConfig) -> Sensor<SimpleFilter> {
    let capacity = 100;
    let smoothing = 0.1;
...
fn process_sensor_data(sensors: &Vec<Arc<Mutex<Sensor<SimpleFilter>>>>, stop: Arc<AtomicBool>) -> anyhow::Result<()> {
    loop {
        if stop.load(Relaxed) {
            break;
        }
